...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awprism
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awprism-*.whl dist/*.tar.gz ...]
...
#: scans. __init__ is the entry point; cli is the command-line interface; prism
#: is the core functionality; registry is the catalogue.
_KEYSTONES = ("awprism/__init__.py", "awprism/cli.py", "awprism/prism.py", "awprism/registry.py")
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awprism-0.0.0-py3-none-any.whl"
    ...
    check("MOAT001 catches a monorepo import",
          any(f.startswith("MOAT001") for f in
              inspect(wheel({**clean, "awprism/x.py": "from lib.core import X\n"}))), True)

    check("MOAT002 catches a debt id",
          any("debt-ledger row id" in f for f in
              inspect(wheel({**clean, "awprism/x.py": "# see D-0000\n"}))), True)

    check("MOAT002 catches a checker rule id",
          any("checker rule id" in f for f in
              inspect(wheel({**clean, "awprism/x.py": "# MOAT001 says so\n"}))), True)
    ...
    check("MOAT003 refuses an artifact missing a keystone",
          any(f.startswith("MOAT003") for f in
              inspect(wheel({"awprism/cli.py": "x=1\n"}))), True)

    # The package's own imports should not be flagged.
    check("does NOT flag the package's own imports",
          inspect(wheel({**clean, "awprism/y.py": "from awprism.models import Model\n"})) == [],
          True)
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awprism-0.0.0.tar.gz"
